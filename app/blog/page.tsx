...
// TS: tell the compiler what our JSON is
...
function useTagFilter() {
  ...
  const activeTag: Tag | '' = TAGS.includes(raw as Tag) ? (raw as Tag) : '';
...
function BlogList({ activeTag }: { activeTag: Tag | '' }) {
      ...
      {filtered.map((post) => {
        ...
        return (
          <li key={post.slug} className="space-y-1">
            ...
            {/* Only link tags that are in our static list */}
